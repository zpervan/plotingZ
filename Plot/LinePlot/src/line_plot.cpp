...
void LinePlot::CreateLinePlot() {
    ...
    for (const auto &data : input_data) {
        for (std::size_t i{0}; i < data.size() - 1; i++) {
            const auto &normalized_origin_point = NormalizeDataPoint(data.at(i));
            const auto &origin_point_screen_position = ConvertUtility::ConvertNormalizedPointToAxisScreenSpace(
                    normalized_origin_point);

            const auto &normalized_end_point = NormalizeDataPoint(data.at(i + 1));
            const auto &end_point_screen_position = ConvertUtility::ConvertNormalizedPointToAxisScreenSpace(
                    normalized_end_point);

            const auto &line_length = MathUtility::CalculateDistanceBetweenTwo2DPoints(
                    origin_point_screen_position,
...
sf::Vector2f LinePlot::NormalizeDataPoint(const sf::Vector2f &data_point) const {
    const auto &max_x_marker_value = plotting_data_->GetMaxXMarkerValue();
    const float normalized_point_x = ConvertUtility::NormalizeValue(data_point.x, max_x_marker_value);

    const auto &max_y_marker_value = plotting_data_->GetMaxYMarkerValue();
    const float normalized_point_y = ConvertUtility::NormalizeValue(data_point.y, max_y_marker_value);
