...
std::vector<sf::RectangleShape> Axis::CreateXAxisMarkerShapes()
{
    ...
    const auto marker_count{x_axis_marker_count_ - 1};

    // Offset the marker position from the origin
    const float offset{0.025};
    const float offset_value{(Config::X_AXIS_DIMENSION.x * offset) + Config::AXIS_LINE_THICKNESS};
    const float marker_axis_length{Config::X_AXIS_DIMENSION.x * (1.0f - offset * 2)};

    for (std::size_t i{0}; i <= marker_count; i++)
    {
        sf::RectangleShape axis_marker{CreateAxisSkeletonShape(Config::MARKER_DIMENSION, 90.0)};

        const float marker_position{static_cast<float>(i) / marker_count};
        sf::Vector2f x_offset{(marker_axis_length * marker_position) + offset_value, 0};
...
std::vector<sf::RectangleShape> Axis::CreateYAxisMarkerShapes()
{
    ...
    // marker shape position percentage!
    const auto marker_count{y_axis_marker_count_ - 1};

    // Offset the marker position from the origin
    const float offset{0.025};
    const float offset_value{(Config::Y_AXIS_DIMENSION.y * offset) + Config::AXIS_LINE_THICKNESS};
    const float marker_axis_length{Config::Y_AXIS_DIMENSION.y * (1.0f - offset * 2)};
    ...
    for (std::size_t i{0}; i <= marker_count; i++)
    {
        ...
        const float marker_position{static_cast<float>(i) / marker_count};
        sf::Vector2f y_offset{0, -(marker_axis_length * marker_position) - offset_value};
