...
sf::Vector2f ScatterPlot::CalculateDataPointPosition(const sf::Vector2f &normalized_point) const {
  const float offsetted_reference_point_x{Config::REFERENCE_POINT.x -
                                          Config::ScatterPlot::DATA_POINT_X_OFFSET};
  const float data_point_position_x{offsetted_reference_point_x +
                                    Config::X_AXIS_DIMENSION.x * normalized_point.x};

  const float offsetted_reference_point_y{Config::REFERENCE_POINT.y -
                                          Config::ScatterPlot::DATA_POINT_Y_OFFSET};
  const float data_point_position_y{offsetted_reference_point_y -
                                    Config::Y_AXIS_DIMENSION.y * normalized_point.y};
